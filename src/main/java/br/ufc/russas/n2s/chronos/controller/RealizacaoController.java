...
public class RealizacaoController {
	...
	public String editarRealizacao(@PathVariable long codAtividade, @PathVariable long codRealizacao,
			@ModelAttribute("realizacao") @Valid RealizacaoBeans realizacao, BindingResult result, Model model,
			HttpServletResponse reponse, HttpServletRequest request) throws IllegalAccessException {
		AtividadeBeans atividadeBeans = atividadeServiceIfc.getAtividade(codAtividade);
		HttpSession session = request.getSession();
		...
		// DATAS
		LocalDateTime dataInicial = LocalDateTime.of(Integer.parseInt(dataI[0]), Integer.parseInt(dataI[1]),
				Integer.parseInt(dataI[2]), Integer.parseInt(horaI[0]), Integer.parseInt(horaI[1]));
		LocalDateTime dataFinal = LocalDateTime.of(Integer.parseInt(dataF[0]), Integer.parseInt(dataF[1]),
				Integer.parseInt(dataF[2]), Integer.parseInt(horaF[0]), Integer.parseInt(horaF[1]));
		if (atividadeBeans != null) {
			try {
				for (Iterator<RealizacaoBeans> iterator = atividadeBeans.getRealizacao().iterator(); iterator
						.hasNext();) {
					RealizacaoBeans realizacaoAUX = iterator.next();
					if (realizacaoAUX.getCodRealizacao() == codRealizacao) {
